...
describe("Types", () => {
  describe("Coordinate", () => {
    ...
    describe("isWithinBoundaries", () => {
      ...
      let assertIsWithinBoundaries = (longitude, result) => {
        expect(Coordinate.isWithinBoundaries(someBoundaries, longitude->Coordinate.longitude_from_float)) 
        |> toEqual(result);
      };

      test("Gives false when longitude is below boundaries min", () => {
        assertIsWithinBoundaries(-100., false);
      });
      ...
      test("Gives true when longitude is above boundaries min", () => {
        assertIsWithinBoundaries(10., true);
      });

      test("Gives true when longitude is below boundaries max", () => {
        assertIsWithinBoundaries(80., true);
      });
    });
  });
});
